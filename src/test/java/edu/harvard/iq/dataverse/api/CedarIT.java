...
public class CedarIT
{
    
    private static final Logger logger = Logger.getLogger(CedarIT.class.getCanonicalName());

    public static final String API_TOKEN_HTTP_HEADER = "X-Dataverse-key";
    
    private static final String TEST_FOLDER_NAME = "cedarCedarITFolder";
    ...
    public void initCEDAR() {
        try {
            ...
            String params = Files.readString(Paths.get("src/test/resources/cedar/cedarParams.json"));
            ...
            logger.severe("Before running ArpCedarIT test, create a cedarParams.json file in the src/test/resources/cedar folder based on cedarParams.example.json.");
            e.printStackTrace();
            fail();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void uploadCitation() {
        try {
            ...
            exportAndCheckTemplate("citation", "Citation Metadata", "src/test/resources/cedar/citation.json", httpClient, apiToken);    
        } catch (Exception e) {
            e.printStackTrace();
            assertEquals(0,1);
        }
    ...
    public void uploadGeospatial() {
        try {
            HttpClient httpClient = getUnsafeHttpClient();
            String apiToken = createRandomSuperUser();
            exportAndCheckTemplate("geospatial", "Geospatial Metadata", "src/test/resources/cedar/geospatial.json", httpClient, apiToken);
        } catch (Exception e) {
            e.printStackTrace();
            assertEquals(0,1);
        }
    ...
    public void uploadSocialScience() {
        try {
            HttpClient httpClient = getUnsafeHttpClient();
            String apiToken = createRandomSuperUser();
            exportAndCheckTemplate("socialscience", "Social Science and Humanities Metadata", "src/test/resources/cedar/socialscience.json", httpClient, apiToken);
        } catch (Exception e) {
            e.printStackTrace();
            assertEquals(0,1);
        }
    ...
    public void uploadAstrophysics() {
        try {
            HttpClient httpClient = getUnsafeHttpClient();
            String apiToken = createRandomSuperUser();
            exportAndCheckTemplate("astrophysics", "Astronomy and Astrophysics Metadata", "src/test/resources/cedar/astrophysics.json", httpClient, apiToken);
        } catch (Exception e) {
            e.printStackTrace();
            assertEquals(0,1);
        }
    ...
    public void uploadBiomedical() {
        try {
            HttpClient httpClient = getUnsafeHttpClient();
            String apiToken = createRandomSuperUser();
            exportAndCheckTemplate("biomedical", "Life Sciences Metadata", "src/test/resources/cedar/biomedical.json", httpClient, apiToken);
        } catch (Exception e) {
            e.printStackTrace();
            assertEquals(0,1);
        }
    ...
    public void uploadJournal() {
        try {
            HttpClient httpClient = getUnsafeHttpClient();
            String apiToken = createRandomSuperUser();
            exportAndCheckTemplate("journal", "Journal Metadata", "src/test/resources/cedar/journal.json", httpClient, apiToken);
        } catch (Exception e) {
            e.printStackTrace();
            assertEquals(0,1);
        }
    ...
    static Response exportMdb(String apiToken, String mdbIdtf, String body) {
        return given()
                ...
                .post("/api/cedar/exportMdbToCedar/" + mdbIdtf);
    }
    ...
    private static String createFolder(String parentFolderId, String cedarDomain, String apiKey, HttpClient client) throws Exception {
        ...
        if (createFolderResponse.statusCode() != 201) {
            throw new Exception("An error occurred during creating new folder: cedarCedarITtestFolder: "+createFolderResponse.body());
        }
